...
Identify every room. For each room provide:
- name: common real estate name (e.g. "Great Room", "Kitchen", "Primary Suite")
- id: slug (e.g. "great-room", "kitchen")
...
Return: {"rooms": [{"id":"...","name":"...","floor":1,"adjacentTo":["..."]}]}` }
  ...
  const result = await httpsRequest({
    ...
    headers: { "x-api-key": claudeKey, "anthropic-version": "2023-06-01", "content-type": "application/json", "content-length": Buffer.byteLength(payload) }
  }, payload);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── stage image (synchronous — gpt-image-1 edits returns synchronously) ──
    ...
    if (!imageBase64 || !stagingPrompt) return { statusCode: 400, headers, body: JSON.stringify({ error: "Missing required fields" }) };

    const imageBuffer = Buffer.from(imageBase64, "base64");
    const boundary = "----FormBoundary" + Math.random().toString(36).slice(2);
    const formData = buildMultipart(boundary, imageBuffer, mimeType || "image/jpeg", stagingPrompt, quality || "low");

    const result = await httpsRequest({
      hostname: "api.openai.com", path: "/v1/images/edits", method: "POST",
      headers: { Authorization: `Bearer ${openaiKey}`, "Content-Type": `multipart/form-data; boundary=${boundary}`, "Content-Length": formData.length }
    }, formData);
    ...
    if (!stagedBase64) {
      console.error("No b64_json:", JSON.stringify(result.body).slice(0, 300));
